...
const useGetCurrentWorkspaceApps = ({
  page_size = 10,
  search,
}: {
  page_size: number;
  ...
  return useInfiniteQuery({
    queryKey: QUERY_KEYS.workspaceAPI.getCurrentWorkspaceApps(
      currentWorkspaceId!,
      page_size,
      search,
    ),
    queryFn: ({ pageParam = 1 }) =>
      workspaceAPI.getWorkspaceApps(pageParam, page_size, search),
...
export default useGetCurrentWorkspaceApps;
