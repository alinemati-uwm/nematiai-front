...
const useSetFavChat = (conversation_uuid: string) => {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: ({ conversation_uuid }: ChatBotAPIRequest["favConversation"]) =>
      chatBotAPI.favConversation({
