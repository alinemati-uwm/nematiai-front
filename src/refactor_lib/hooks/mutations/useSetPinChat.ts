...
const useSetPinChat = (conversation_uuid: string) => {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: ({ conversation_uuid }: ChatBotAPIRequest["pinConversation"]) =>
      chatBotAPI.pinConversation({
