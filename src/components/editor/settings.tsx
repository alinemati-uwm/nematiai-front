...
export function SettingsProvider({ children }: { children: ReactNode }) {
  ...
  return (
    <SettingsContext value={{ keys, model, setKey, setModel }}>
      {children}
    </SettingsContext>
  );
}
  ...
  const renderApiKeyInput = (service: string, label: string) => (
    <div className="group relative">
      <div className="flex items-center justify-between">
        <label
          className="absolute top-1/2 block -translate-y-1/2 cursor-text px-1 text-sm text-label-light/70 transition-all group-focus-within:pointer-events-none group-focus-within:top-0 group-focus-within:cursor-default group-focus-within:text-small group-focus-within:font-medium group-focus-within:text-label has-[+input:not(:placeholder-shown)]:pointer-events-none has-[+input:not(:placeholder-shown)]:top-0 has-[+input:not(:placeholder-shown)]:cursor-default has-[+input:not(:placeholder-shown)]:text-small has-[+input:not(:placeholder-shown)]:font-medium has-[+input:not(:placeholder-shown)]:text-label"
          htmlFor={label}
        >
          <span className="inline-flex bg-holder-lighter px-2">{label}</span>
        </label>
        <Button
          asChild
          size="icon"
          variant="ghost"
          className="absolute end-6 top-0 h-full"
        >
          <Link
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        ...
        <form className="space-y-10" onSubmit={handleSubmit}>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-4">
              ...
              <div className="group relative">
                <label
                  className="absolute start-1 top-0 z-10 block -translate-y-1/2 bg-holder-lighter px-2 text-small font-medium text-label group-has-[:disabled]:opacity-50"
        ...
        <p className="text-sm text-label-light">
