...
import { globalCSS as css } from "markmap-view";

import { createPathD } from "@/components/pages/mind-map/lib/edges";
...
export class Markmap {
  ...
  private _relayout() {
    ...
    const { paddingX, spacingHorizontal, spacingVertical, structure } =
      this.options;
    let verticalLayout: MarkmapStructure[] = [
      "organization",
      "organization_upward",
      "timeline",
    ];
    const spaceX = verticalLayout.includes(structure)
      ? spacingVertical
      : spacingHorizontal;
    const spaceY = verticalLayout.includes(structure)
      ? spacingHorizontal
      : spacingVertical;
    ...
    const layout = flextree<INode>({})
      .nodeSize(node => {
        ...
        return [height, width + (width ? paddingX * 2 : 0) + spaceX];
      })
    ...
    rePoseNodes(fnodes, structure, this.state.rect, spaceX);
  ...
  async renderData(originData?: INode) {
    ...
    mmCircleMerge
      .attr("cx", d => getCirclePosition(d, structure).cx)
      .attr("cy", d => getCirclePosition(d, structure).cy);

    const isMindMap = structure === "mind_map";
    this.transition(mmCircleMerge)
      .attr("r", d => (isMindMap && d.state.depth === 1 ? 0 : 6))
      .attr("stroke-width", d =>
        isMindMap && d.state.depth === 1 ? "0" : "1.5",
      );
    ...
    this.transition(mmPathMerge)
      ...
      .attr("d", d => {
        const { pathType, structure } = this.options;
        const { source, target } = getPathPositions(d, structure);
        const side = (
          d.target.payload ? d.target.payload.side || "right" : "right"
        ) as "left" | "right";
        return createPathD(
          source,
          target,
          pathType,
          structure,
          structure === "logical_left" ||
            (structure === "mind_map" && side === "left"),
        );
      });
